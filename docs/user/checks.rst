...
AI quality checks are populated asynchronously by the
:ref:`AI quality evaluation add-on <addon-weblate.ai.quality>`. Each category groups
the evaluator's findings, including their severity and explanation, and uses the
same filtering and ignore controls as other checks. AI evaluation runs only
during add-on runs, according to its configured triggers. Regular quality check
processing does not contact the evaluator or create new AI findings; it only
retains existing findings or removes them when the source or translation changes.
