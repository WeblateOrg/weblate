...
Add-ons that expose a component API show its base URL on the add-on's
:guilabel:`API` tab, alongside a link to the API root.
...
The :ref:`addon-weblate.autotranslate.autotranslate` mode
:guilabel:`Add as approved translation` creates approved strings when reviews are
enabled for the target language. If the project settings or a language workflow
override disable reviews, it creates translated strings instead. The configured
mode is preserved, including for project-wide and site-wide add-ons whose targets
