...
settings. Event context does not automatically limit actions to the event's
language or affected units; each action selects strings using its own query.
...
Provide configured engine IDs when selecting ``mt``.
...
Uses the same operation as :ref:`addon-weblate.flags.bulk`. The required ``q``
selects strings using :ref:`search-strings`. ``state`` sets an integer string state,
or leaves it unchanged with -1 (the default).
