...
:Summary: The string uses unsafe HTML markup.
:Scope: source and translated strings
...
The string uses unsafe HTML markup. This check has to be enabled using the
``safe-html`` flag (see :ref:`custom-checks`). Event-handler attributes in
source strings are reported because their values would otherwise be
translatable. The accompanying autofixer removes them and other unsafe markup
from translations.
