...
limits are configurable. Keep manifests revalidated as described below.
...
Configure the CDN origin to serve manifests with revalidation (for example,
``Cache-Control: no-cache``), and artifact files with
...
bundled resources. Retired manifests return 404. Hashes detect corrupted or
