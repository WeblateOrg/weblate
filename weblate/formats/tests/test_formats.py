...
class XWikiPropertiesFormatTest(PropertiesFormatTest):
    ...
    def test_save(self, edit=False):
        ...
        self.assertIn(
            "# This contains the translations of the module in the default "
            "language\n"
            "# (generally English).",
            units[1].notes,
        )
        ...
        self.FORMAT.add_language(
            translation_file, Language.objects.get(code="cs"), self.BASE
        )
        translation_data = self.FORMAT(
            storefile=translation_file, template_store=storage, language_code="cs"
        )
        ...
        if edit:
            ...
            unit_to_translate, create = translation_data.find_unit(
                units[1].context, units[1].source
            )
            ...
            # Check encoding
            unit_to_translate, create = translation_data.find_unit(
                units[2].context, units[2].source
            )
            self.assertTrue(create)
            translation_data.add_unit(unit_to_translate.unit)
            ...
            unit_to_translate, create = translation_data.find_unit(
                units[5].context, units[5].source
            )
        ...
        with open(translation_file, "rb") as handle:
            newdata = force_str(handle.read(), encoding="iso-8859-1")
        ...
        if edit:
            ...
            self.assertFalse(units[1].unit.isheader())
            self.assertFalse(units[1].unit.isblank())
            self.assertEqual("Confirmation de l'opération {0}", units[1].target)
...
class XWikiPagePropertiesFormatTest(PropertiesFormatTest):
    ...
    def test_save(self, edit=False):
        ...
        self.assertEqual(
            "administration.section.users.disableUser.done", units[0].source
        )
        ...
        self.assertTrue(units[1].has_content())
        self.assertEqual(
            "administration.section.users.disableUser.failed", units[1].source
        )
        ...
        self.assertEqual(
            "administration.section.programmingRightsWarning", units[2].source
        )
        self.assertEqual(
            "The user you're about to delete is the last author of "
        ...
        if edit:
            ...
            self.assertIn(
                "# Missing: administration.section.users.disableUser.failed="
...
class XWikiFullPageFormatTest(AutoFormatTest):
    ...
    def test_save(self, edit=False):
        ...
        # first unit is the content
        self.assertTrue(units[0].has_content())
        self.assertEqual("content", units[0].source)
        self.assertIn("= Headings =", units[0].target)
        self.assertIn("* [[Sandbox Test Page 1>>Sandbox.TestPage1]]", units[0].target)
        self.assertIn(
            "{{info}}\nDon't worry about overwriting\n{{/info}}", units[0].target
        )

        # second unit is the title
        self.assertTrue(units[1].has_content())
        self.assertEqual("title", units[1].source)
        self.assertEqual("Sandbox", units[1].target)
        ...
        if edit:
            # Put a simple quote to see if the serialization is properly handled
            unit_to_translate, create = translation_data.find_unit(
                units[0].context, units[0].source
            )
            self.assertTrue(create)
            translation_data.add_unit(unit_to_translate.unit)
            translation_data.all_units[0].unit = unit_to_translate.unit
            unit_to_translate.set_target(
                "= Titre=\n"
                "\n"
