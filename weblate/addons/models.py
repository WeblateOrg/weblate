    ...
    for addon in addons:
        ...
        try:
            with sentry_sdk.start_span(
                op=f"addon.{event_type}", description=addon.name
            ):
...
def pre_push(sender, component, **kwargs):
    handle_addon_event(
        component,
        EVENT_PRE_PUSH,
        lambda addon, comp, component=component: addon.pre_push(comp),
        "pre_push",
    )
...
def post_push(sender, component, **kwargs):
    handle_addon_event(
        component,
        EVENT_POST_PUSH,
        lambda addon, comp, component=component: addon.post_push(comp),
        "post_push",
    )
    ...
    **kwargs,
    ...
    if event_type:
        handle_addon_event(
            component,
            event_type,
            lambda addon, comp, component=component: addon_method(addon, comp),
            "post_update",
        )
...
def component_update(sender, component, **kwargs):
    handle_addon_event(
        component,
        EVENT_COMPONENT_UPDATE,
        lambda addon, comp, component=component: addon.component_update(comp),
        "component_update",
    )
...
def pre_update(sender, component, **kwargs):
    handle_addon_event(
        component,
        EVENT_PRE_UPDATE,
        lambda addon, comp, component=component: addon.pre_update(comp),
        "pre_update",
    )
...
def pre_commit(sender, translation, author, **kwargs):
    handle_addon_event(
        translation,
        EVENT_PRE_COMMIT,
        lambda addon, trans, translation=translation: addon.pre_commit(trans, author),
        "pre_commit",
    )
...
def post_commit(sender, component, **kwargs):
    handle_addon_event(
        component,
        EVENT_POST_COMMIT,
        lambda addon, comp, component=component: addon.post_commit(comp),
        "post_commit",
    )
...
def post_add(sender, translation, **kwargs):
    handle_addon_event(
        translation,
        EVENT_POST_ADD,
        lambda addon, trans, translation=translation: addon.post_add(trans),
        "post_add",
    )
...
def unit_pre_create_handler(sender, unit, **kwargs):
    handle_addon_event(
        unit.translation,
        EVENT_UNIT_PRE_CREATE,
        lambda addon, uni, unit=unit: addon.unit_pre_create(uni),
        "unit_pre_create",
    )
...
def unit_post_save_handler(sender, instance, created, **kwargs):
    handle_addon_event(
        ...
        EVENT_UNIT_POST_SAVE,
        lambda addon, inst, instance=instance: addon.unit_post_save(inst, created),
        "unit_post_save",
    )
...
def store_post_load_handler(sender, translation, store, **kwargs):
    handle_addon_event(
        translation,
        EVENT_STORE_POST_LOAD,
        lambda addon, trans, translation=translation: addon.store_post_load(
            trans, store
        ),
        "store_post_load",
    )
