    ...
    scopes = tuple(
        ProjectRepositoryScope(
            repository=components_by_id[
                owner_id
                if owner_id in project_components_by_owner[owner_id]
                else project_components_by_owner[owner_id][0]
            ],
            project_components=tuple(
                components_by_id[component_id]
                for component_id in project_components_by_owner[owner_id]
            ),
            permission_components=tuple(permission_components_by_owner[owner_id]),
        )
        for owner_id in project_components_by_owner
    )
...
def get_project_repository_selection(
    ...
    allowed_permissions = tuple(
        ...
        if check_permission(user, permission, project)
    )
