...
class Ollama(BatchMachineTranslation):
    """
    ...
    def _download(
        ...
        *,
        ...
        add_breadcrumb("ollama", "prompt", prompt=prompt)
        add_breadcrumb("ollama", "chat", content=content)
        ...
        try:
            ...
            api_url = urljoin(self.settings["url"], GENERATE_ENDPOINT)
            response = requests.request("post", api_url, json=payload)
        except Exception as error:
            raise MachineTranslationError(error)

        translations_string = response.json().get("response")
        add_breadcrumb("ollama", "response", translations_string=translations_string)
