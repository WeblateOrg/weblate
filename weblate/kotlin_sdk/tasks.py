...
def publish_kotlin_sdk(addon_id: int) -> None:
    ...
    if candidate is None or candidate.component is None:
        return
    with WeblateLock(scope="kotlin-sdk", key=addon_id, slug=str(addon_id)):
        with transaction.atomic():
            addon = Addon.objects.select_for_update().filter(pk=addon_id).first()
            if addon is None or not addon.is_valid or addon.api_name != "kotlin-sdk":
                return
        ...
        try:
            outcome = Publication(addon).publish()
