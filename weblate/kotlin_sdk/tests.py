...
from weblate.kotlin_sdk.arsc import Text
...
if TYPE_CHECKING:
    ...
    from weblate.kotlin_sdk.arsc import ResourceTable
...
class RegistrationConcurrencyTest(RepoTestMixin, TransactionTestCase):
    ...
    def test_lifecycle_changes_during_compilation(self) -> None:
        ...
        from weblate.kotlin_sdk.arsc import generate  # ruff: ignore[import-outside-top-level]
        from weblate.kotlin_sdk.tasks import publish_kotlin_sdk  # ruff: ignore[import-outside-top-level]
        ...
        for action in ("register", "retire", "uninstall", "reinstall", "expire"):
            with self.subTest(action=action):
                ...
                def compile_locale(
                    ...
                    barrier.wait(timeout=10)
                    return generate(package, locale, resources)
                ...
                with (
                    ...
                    patch(
                        "weblate.kotlin_sdk.publication.generate",
...
class KotlinSDKTest(ViewTestCase):
    ...
    def test_shared_preparation(self) -> None:
        ...
        with (
            ...
            patch(
                "weblate.kotlin_sdk.publication.generate", wraps=generate
            ) as compile_locale,
            patch.object(
                addon, "refresh_inventory", wraps=addon.refresh_inventory
            ) as inventory,
    ...
    def test_artifact_staging_and_memory_bound(self) -> None:
        ...
        def compile_locale(
            package: str, locale: str, resources: ResourceTable
        ) -> memoryview:
        ...
        with (
            patch(
                "weblate.kotlin_sdk.publication.generate", side_effect=compile_locale
            ),
            self.captureOnCommitCallbacks(execute=True),
        ):
            self.stage_build(addon, build, locales(snapshot))
        ...
        with (
            ...
            patch("weblate.kotlin_sdk.publication.generate") as generate,
