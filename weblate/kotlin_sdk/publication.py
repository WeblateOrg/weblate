...
class ArtifactInventory:
    ...
    def capacity(self, outputs: dict[str, int]) -> tuple[int, int]:
        return (
            self.total
            + sum(size - self.sizes.get(name, 0) for name, size in outputs.items()),
            len(self.sizes) + sum(name not in self.sizes for name in outputs),
        )


class Publication:
    def __init__(self, instance: Addon, *, lock: WeblateLock | None = None) -> None:
    ...
    def locked_addon(self) -> Generator[Addon | None]:
        ...
        with transaction.atomic():
            ...
            if (
                ...
                return
            self.instance = addon
            if not addon.addon.can_process(component=addon.component):
                now = timezone.now()
                for build in addon.sdk_builds.filter(retired__isnull=True).defer(
                    "metadata"
                ):
    ...
    def commit(self, build: BuildInput, prepared: PreparedPublication) -> None:
        ...
        try:  # ruff: ignore[too-many-statements-in-try-clause]
            with self.locked_addon() as addon:
                ...
                total, count = inventory.capacity(sizes)
                if total > MAX_ARTIFACT_BYTES or count > MAX_ARTIFACT_FILES:
                    inventory = self.refresh_inventory()
                    total, count = inventory.capacity(sizes)
                self.check_artifact_capacity(total=total, count=count)
                ...
                for digest, (path, size) in prepared.outputs.items():
                    ...
                    elif inventory.sizes[target.name] != size:
                        self.cdn.copy_cdn_file(str(path), f"artifacts/{target.name}")
                        inventory.total += size - inventory.sizes[target.name]
                        inventory.sizes[target.name] = size
                artifacts = {
    ...
    def finalize_manifest(self, build_id: int) -> str | None:
        ...
        with self.locked_addon() as addon:
            ...
            try:  # ruff: ignore[too-many-statements-in-try-clause]
                ...
                build.save(update_fields=["status", "error", "pending_manifest"])
        ...
        with temporary as directory:
            try:
                prepared = prepare(
                    ...
                    renew_lock=self.lock.reacquire if self.lock is not None else None,
                )
            except (
                ValueError,
